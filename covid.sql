...
CREATE TABLE iati_data_with_covid_flag AS SELECT iati_data.*, COALESCE(CAST(covid_flag_mapping.covid_flag AS INTEGER), 0) AS covid_flag, COALESCE(CAST(covid_flag_mapping.covid_flag_strict AS INTEGER), 0) AS covid_flag_strict FROM iati_data LEFT JOIN covid_flag_mapping ON iati_data.iati_identifier=covid_flag_mapping.iati_identifier;
